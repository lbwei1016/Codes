...
struct seg_tree {
    int val, tag; // tag; all 0 or all 1
    // int l, r;
    seg_tree *lch, *rch;
    seg_tree(seg_tree *lc, seg_tree *rc): lch(lc), rch(rc), val(0), tag(0) {};
    seg_tree(int v=0): val(v), tag(0) {};
    void pull() { val += lch->val + rch->val; };
};

void modify(seg_tree *s, int l, int r, int ml, int mr, int val) {
    if(ml == mr) return;
    if(ml<=l && r<=mr) s->tag = val;
    else {
        int mid = (l + r) / 2;
        ...
        if(val == 0) {
            s->val -= min(r, mr)-max(l, ml);
            s->val = max(0, s->val);
        }
        else {
            s->val += min(r, mr)-max(l, ml);
            s->val = min((min(r, mr)-max(l, ml)), s->val);
        }
    }
}

int query(seg_tree *s, int l ,int r, int ql, int qr) {
    if(s->tag == 1) return 1;
...
void solve(vector<int> &v, vector<int> &C, vector<int> &L, vector<int> &R) {
    ...
    // cout << seg->val;
    for(int i=0; i<q; ++i) {
        int num1 = query(seg, 0, pow2, L[i], R[i]);
