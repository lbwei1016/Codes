...
***String*** --  Gusfield’s Algorithm, a.k.a "Z algorithm"
    Description:
        ...
        Z(i) 的意義：
            ...
            開頭盡可能長得一樣。
        實作：
            要進行字串匹配的話，只要令字串　C = B + $ + A，其中 $ 是一個從
            未在 A, B 中出現過的字元，然後看是否存在 0 <= k < LA 使得 
            Zc(LB + 1 + k) = LB 就可以了。
            （事實上，若將 B 的 Z　函數與 A 的 Z 函數分開計算就不必宣告新的串接兩者的字串 C）
    O(La + Lb)
        Li: i 字串長度
*/
#include <iostream>
...
int match(const string A, const string B) {
    ...
    // 中間的 '@' 可替換為任意未在 A, B 中出現的字元
    // 另外，實作時未必真的要將 A, B 串接起來
    string C = B + '@' + A; 
    ...
    for(int i=1; i<C.size(); ++i) {
        ...
        if(Z[i] == B.size()) return i;
    }
    return -1;
}
// 更精簡的寫法
int match_2(const string A, const string B) {
    int Z[MAX_C]; // 「Z 函數」
    ...
    for(int i=1; i<C.size(); ++i) {
        ...
        else Z[i] = min(Z[bst]+bst-i, Z[i-bst]); // 畫圖看看！
        /*
