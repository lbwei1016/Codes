...
***Minimum_Cost_s-t_Flow*** --Dijkstra
    ...
    Prerequisite:
        ...
        Then, define: d'(e) = d(e) + h(u) - h(v) >= 0；Since new cost "d'(v)" is never 
...
typedef pair<int, int> P; //first: shortest path；second: No. of vertices
...
int h[MAX_V]; //potential；h[v] is the shortest path from "s" to "v"
int dist[MAX_V]; // modified shortest path；dist[e.to] = dist[v] + e.cost + h[v] - h[e.to]
...
int min_cost_flow(int s, int t, int f) {
    ...
    while(f > 0) {
        ...
        /*
            update: dist[v] = Σe_cost - h[v]
            => new_h[v] = h[v] + dist[v] = h[v] + (Σe_cost - h[v]) = Σe_cost = shortest distance
        */
        for(int v=0; v<V; v++) {
            h[v] += dist[v];
        }
