...
int main() {
    ...
    for (int i=0; i<n; ++i) {
        if (dp[0][i][0] < mn) {
            mns.clear();
            mns.push_back({.at=i, .dir=1});
            mn = dp[0][i][0];
            // at = i;
            // dir = 1; // 相反
        }
        else if (dp[0][i][0] == mn) mns.push_back({.at=i, .dir=1});

        if (dp[0][i][1] < mn) {
            mns.clear();
            mns.push_back({.at=i, .dir=0});
            mn = dp[0][i][1];
            // at = i;
            // dir = 0;
        }
        else if (dp[0][i][1] == mn) mns.push_back({.at=i, .dir=0});
    }

    string mn_res;
    for (int i=0; i<1000; ++i) mn_res.push_back('z');
    for (auto X: mns) {
        int at = X.at, dir = X.dir;
        int R = 0, cnt = 0;
